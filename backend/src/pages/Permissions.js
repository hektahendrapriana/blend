...
function Permissions() {
  ...
  const getDataList = async () => {
    await Api().get(`permissions?filter=${keyword}&fields=role_id,page_id&page=${page}&limit=${limit}`)
          ...
          <div className="content-header">
            <Container fluid>
              <Row className="">
                ...
                <Col sm={2}>
                    { checkMenu('addpermission') &&
                        <Link to="addpermission"><Button variant="outline-primary" className="btn-upload"><AiOutlinePlus /> Add Permission</Button></Link>
                    }
                </Col>
                <Col sm={3}>
                  ...
                  <Table striped hover responsive>
                    <thead className="thead">
                      <tr>
                        ...
                        <th style={{ width: '25%'}} className="text-left">Role Name</th>
                        <th style={{ width: '15%'}} className="text-left">Role</th>
                    ...
                    <tbody>
                      {
                        dataList.map((item, i) => (
                          <tr key={i}>
                            ...
                            <td className="text-left">{item.role_id.role}</td>
                            ...
                            <td className="text-left">
                              <NavDropdown title="Action" className='' id="basic-nav-dropdown">
                                  { checkMenu('editpermission') && 
                                      <NavDropdown.Item href={ "editpermission/" + item._id }><AiFillEdit /> Change</NavDropdown.Item>
                                  }
                                  { checkMenu('deletepermission') && 
